...
reactable(
  as.data.frame(
    lapply(
      get_liquidity(x = as.bigz(1.146808312350982128*1e18),
                    y = as.bigz(0.00493069417466594*1e18),
                    P = sqrtpx96_to_price('7609891887904192334730581214'),
                    pa = tick_to_price(-50100),
                    pb = tick_to_price(-39120),
                    yx = TRUE), as.character
    )
  )
)
...
```{r}
validation_initial$price <- sqrtpx96_to_price('6786861222905019303556225263')

validation_positions <- validation_initial %>% mutate(
    active = ifelse(price >= min_price & price <= max_price, TRUE, FALSE)
)
...
- `swap_across_ticks()`: Give a position table and a desired swap size, it iterates through identifying the recalculation price, swapping to that price, checking the positions, and restarting until the swap is complete.
 ...
 if(is.null(dx) & is.null(dy)){
  ...
  if(is.null(dx)){
    ...
    if(max_y >= amount){
     ...
     swap = swap_within_tick(L = current_L, sqrtpx96 = price_to_sqrtpx96(price), 
                      dy = amount, 
                      decimal_x = decimal_x, 
                      decimal_y = decimal_y, 
                      fee = fee)
      
     fee_tbl$yfee <- fee_tbl$yfee + as.numeric(
     ...
     # swap as much as you can and repeat process
    } else { 
    ...
    amount <- dx
    price <- sqrtpx96_to_price(sqrtpX96 = sqrtpx96)
    recalc_price <- find_recalculation_price(positions = ptbl, 
                                        current_price = price, 
                                        price_up = FALSE)
