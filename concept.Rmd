...
tick_to_price <- function(tick){
  sqrt(1.0001)^(2*tick) * 1e10
}

get_closest_tick <- function(desired_price, tick_spacing = 60){
# base = sqrt(1.0001)
# y = sqrt(price / 1e10)
# base^tick = y
# tick = log(y, base)
# price = 1e10*(base^tick)^2
...
initial_tick <- log( sqrt(desired_price / 1e10), sqrt(1.0001) )  
...
if(initial_tick %% tick_spacing == 0){
  ...
  r$tick <- final_tick 
  r$actual_price <- sqrt(1.0001)^(2*final_tick) * 1e10
}
...
reactable(
  rbind(
    as.data.frame(get_closest_tick(0.05)),
    as.data.frame(get_closest_tick(0.25))
  )
)

```

Spreading liquidity evenly across ticks results in this (uniform) distribution of liquidity.

Given real reserves 33 WBTC and 333 ETH at 0.1 current price;
