...
In Uniswap v3, The entire 0 to Infinite range is broken up into ticks, spaced logarithmically such
that each tick price is 0.01% larger than the previous tick.

Liquidity providers choose their range (e.g., 1 ETH = 0.05 BTC to 1 ETH = 0.25 BTC) and spread their tokens across the ticks. 

These ticks act like 'buckets' of liquidity pooled from multiple providers. As trades occur, price impact happens using the same x*y = k formula within a ticket. Ticks become exhausted of liquidity as price impact moves pushes the relative prices into other buckets of liquidity (i.e., across ticks).

When someone is out of range (e.g., if 1 ETH = 0.049 BTC) their position is 100% in the lower value asset (they sold all their winners to buy the trader's losers with the hope their share of fees makes up for the loss).
...
pb = ETH/BTC at the low tick (low for ETH) and pa = ETH/BTC at the high tick (high for ETH). So for this function we inverse 0.05 to 20 and .25 to 4 to state how many ETH you get for 1 BTC. 
...
l_func <- function(k = 100000, pb = 20, pa = 4, x){
  #' l = sqrt(k)
...
Instead of providing liquidity from 0 to Inf; a liquidity provider bounded between 0.05 and 0.25 
concentrates their liquidity, accepting that *outside their range* they are 100 % in a single asset.
