...
- Maximum Price: 1 ETH = 0.25 BTC (1 ETH gets 5 times as much: 0.25 BTC)
...
Imagine a single liquidity provider wanted to provide *the same liquidity* but with fewer
tokens. They could concentrate their liquidity between 0.05 and 0.25 BTC/ETH.

Note: Using Uniswap v3 White Paper format where the unit of account is Y (here, ETH)

pa = BTC/ETH at the low tick (low for BTC)
pb = BTC/ETH at the high tick (high for BTC)

So for this function we inverse 0.05 to 20 and .25 to 4 to state how many ETH you get for 1 BTC. 

Getting 20 ETH for 1 BTC means ETH is low. This is useful to think about because at this 
point your position is 100% ETH. Getting 4 ETH for 1 BTC means ETH is high. At this point
your position is 100% BTC.


```{r,  warning=FALSE, message=FALSE, fig.show='hide'}

l_func <- function(k = 100000, pa = 4, pb = 20, x){
  #' l = sqrt(k)
...
get_price <- function(k = 100000, pb = 20, pa = 4, x){
   l = sqrt(k)
  f1 = l/sqrt(pb)
  f2 = l*sqrt(pa)
...
# This plots by default: suppressing this figure.
concentrated <- curve(l_func(k = 100000, pb = 20, pa = 4, x = x), from = 0, 100, n = 10000)

```

```{r,  warning=FALSE, message=FALSE}
# negative values don't make sense 
conc_pool <- data.frame(
  btc = concentrated$x,
  eth = concentrated$y
)

conc_pool <- conc_pool %>% dplyr::filter(btc >= 0, eth >= 0)
conc_pool$ethbtc <- conc_pool$eth/conc_pool$btc
pool$ethbtc <- pool$eth/pool$btc

plot_ly(conc_pool, x = ~btc) %>% 
  add_trace(y = ~eth, mode = 'lines', type = 'scatter',
            name = 'Real Reserve', line = list(color = 'orange')) %>% 
  add_trace(x =~pool$btc, y = ~pool$eth, name = 'Virtual Reserve', mode = 'lines', line = list(color = 'blue')) %>% 
  add_trace(x = ~33, y = ~333, name = 'Real 0.1 BTC/ETH', mode = 'markers') %>% 
  add_trace(x = ~100, y = ~1000, name = 'Virtual 0.1 BTC/ETH', mode = 'markers') %>% 
  add_trace(x = ~low$x, y = ~low$y, mode = 'markers', name = '1 ETH = 0.05 WBTC') %>%
  add_trace(x = ~high$x, y = ~high$y, mode = 'markers', name = '1 ETH = 0.25 WBTC') %>%
  layout(title = "Virtual vs Real Reserves for BTC/ETH 0.05 - 0.25",
         xaxis = list(
           range = c(0, 200)
         ),
         yaxis = list(
...
In the Uniswap v2 pool, prices change smoothly following standard x*y = k math described above.
In Uniswap v3, concentrated liquidity creates *real reserves* (yellow) that act as the virtual reserves
within the price boundaries (the two outer dots of the virtual reserve line).

In Uniswap v3, liquidity providers choose their range (e.g., Min 1 ETH = 0.05 BTC to Max 1 ETH = 0.25 BTC) and spread their tokens across the ticks. When someone is out of range (e.g., if 1 ETH = 0.049 BTC) their position is 100% in the lower value asset (they sold all their winners to buy the trader's losers with the hope their share of fees makes up for the loss).
...
Most ERC20 that are not stablecoins use 18 decimals and won't require an adjustment. But wrapped Bitcoin, WBTC, has 8 decimals to mimic Bitcoin; while ETH has 18. Thus, a 1e10 (18-8 = 10) decimal adjustment is needed.
...
Given 1 BTC, the amount of ETH required to concentrate liquidity in the range of 0.05 BTC/ETH (20 ETH / BTC) 
and 0.25 BTC/ETH (4 ETH / BTC) depends on the current price of BTC/ETH (or ETH/BTC).

Using the same syntax from the White Paper where x is BTC and y is ETH; 

- x = 1 BTC
- y = ?? ETH 
- Current Price, P = y/x = 10 ETH/BTC (0.1 BTC/ETH)
- min price (for BTC), pa = 4 ETH/BTC  (BTC is LOW, we are 100% BTC at this price) 
- max price (for BTC), pb = 20 ETH/BTC (BTC is HIGH, we are 100% ETH at this price) 
...
To do the calculation in reverse (matching 12.549 ETH to 1 BTC), switch pb and pa; then inverse P, pb, and pa.

- x = ?? BTC
- y = 12.549 ETH
- Inverse Current Price, P^-1 = x/y = 0.1 BTC/ETH (10 ETH/BTC)
...
- x = 100 BTC
- y = 1000 ETH
- Current Price, P = y/x = 10 ETH/BTC
- min_price = 
