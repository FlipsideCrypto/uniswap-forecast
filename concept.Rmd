...
get_liquidity <- function(x, y, P, pa, pb, yx = TRUE){ 
  ...
  x_f1 <- as.bigq(sqrt(P)) * as.bigq(sqrt(pb))
  x_f2 <- as.bigq(sqrt(pb)) - as.bigq(sqrt(P))
  Lx <- x * x_f1 / x_f2
  
  # get big integer
  r$Lx <- as.bigz(Lx)
  
  y_f1 <- as.bigq(sqrt(P)) - as.bigq(sqrt(pa))
  Ly <- y / y_f1
    # get big integer
  r$Ly <- as.bigz(Ly)
...
swap_xy <- swap_within_tick(L = liquidity$minL,
...
size_price_change_in_tick <- function(L, sqrtpx96, sqrtpx96_target, dx = TRUE, 
 ...
 # given a liquidity and known new price, how much of a change in dx is required to get to the new price
