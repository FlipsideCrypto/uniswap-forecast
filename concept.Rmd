...
When someone is out of range (e.g., if 1 ETH = 0.049 BTC) their position is 100% in the lower value asset (they sold all their winners to buy the trader's losers).

Recall the Simulated 100 BTC; 1000 ETH liquidity pool from before but this time focused
on the range of 60 BTC to 140 BTC.
...
```{r}
# Imagine a single liquidity provider in the range {pa, pb}

# Something is wrong here - is it real reserves transposed to pa/pb and 
# not the reverse? is pb the "virtual" price not the real minimum?
...
kv3_func <- function(x, pa = pa, pb = pb, k = 100000){
  # returns y for an x that meets condition
  # (x + sqrt(k/pb))*(y+sqrt(k*pa)) = k
  
  y = ( k/(x+sqrt(k/pb)) - sqrt(k*pa)  )
...
# draws plot and returns values 
x_v3 <- curve(kv3_func(x = x, pa = pa^-1, pb = pb^-1), 0,1000, n = 1001, xlab = 'BTC', ylab = 'ETH')
...
plot_ly(pool_v3, x = ~btc) %>% 
  add_trace(y = ~eth, mode = 'lines', type = 'scatter', name = 'Real Reserve', line = list(color = 'orange')) %>% 
  add_trace(x =~pool$btc, y = ~pool$eth, name = 'Virtual Reserve', mode = 'lines', line = list(color = 'blue')) %>% 
  layout(title = "Virtual vs Real Reserves for ETH/BTC 0.036 - 0.196")
```
