...
Liquidity providers choose their range (e.g., 1 ETH = 0.05 BTC to 1 ETH = 0.25 BTC) and spread their tokens across the 0.01% buckets. 

Each bucket of 0.01% range has liquidity from multiple providers stacked into it. As trades occur price impact happens using the same x*y = k formula, where buckets are exhausted of liquidity as price impact moves pushes the relative prices into other buckets.
...
l_func <- function(k = 100000, pb, pa, x){
  #' l = sqrt(k)
...
Showing this concentrated range offers the same price impacts as the larger pool 
(i.e., the same relevant liquidity).

```{r}
subconc <- conc_pool %>% filter(ethbtc > 9 & ethbtc < 11)
subpool <- pool %>% filter(ethbtc > 9 & ethbtc < 11)
