...
Unfortunately, the most readily available equations use the inverted price: Y/X (i.e., ETH/BTC) for these calculations, in contrast with the original white paper.

So again, using inversion:

-   x = 100 BTC
-   y = 1000 ETH
-   Current Price, y/x, 10 ETH/BTC
-   max price = 0.25 BTC/ETH => becomes min price 4 ETH/BTC.
-   min price = ?? ETH/BTC => becomes max price ?? ETH/BTC.

We can calculate the inverted max price, and then invert it to find the min price for our preferred (original white paper) x/y price ratio.

```{r}

match_tokens <- function(x, y, P, pa = NULL, pb = NULL, yx = TRUE){
...
match_min_to_tokens <- function(x, y, P, pb){
  #' pa = ( 
  #' ( y / (sqrt(pb)*x) ) + 
  #'  sqrt(P) - 
  #'  ( y / (sqrt(P)*x))
  ...
  r$required_min_price <- pa
  
  return(r)
  
}

match_max_to_tokens <- function(x, y, P, pa){
  #' pa = ( 
  #' ( y / (sqrt(pb)*x) ) + 
  #'  sqrt(P) - 
  #'  ( y / (sqrt(P)*x))
  #'   ) ^2
  #'  Solve the above for pb
...
reactable(
  as.data.frame(
    match_max_to_tokens(x = 100, y = 1000, P = 10, pa = 4)
  )
)

```

Inverting this back to our desire BTC/ETH format:

-   x = 100 BTC
-   y = 1000 ETH
-   Current Price, y/x, 10 ETH/BTC => 0.1 BTC/ETH
-   max price 25 ETH/BTC => 0.04 BTC/ETH.
-   min price 4 ETH/BTC => max price 0.25 BTC/ETH
